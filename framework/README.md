...
Matter is wave, sampled. A single topological postulate produces one scaling law. It reveals the constants of the universe: $\Lambda$, $H_0$, $a_0$, and $\alpha$; and 24 fermion masses across 122 orders of magnitude. The topology determines boundary conditions, selects the cosmic wave $\Psi$, fixes the observer position $\sqrt{\Omega}$, and generates a discrete phase structure native to $S^3$.
...
**What MIT does claim.** One postulate (topology), one domain boundary ($\ell_P$), one calibration layer (well-to-observable assignments). The scaling law $A/A_P \approx C(\Theta) \cdot (\sqrt{\Omega})^{-n}$ and every component in it trace to the topology postulate.
