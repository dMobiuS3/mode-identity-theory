...
**The scale-factor normalization is written two ways, and the split is logged rather than swept.** The effective scale factor carries $`a_\text{eff} = a_\ast S`$ on the [framework page](../../../README.md) and $`a_\text{eff} = a_0 S`$ on [the stress-tensor bridge](stress-tensor-bridge.md) and the pages that cite its placement. The constant is the same object in both. The $`\ast`$ form is the one the rule above recommends, since $`a_0`$ is the acceleration scale across the [SPARC work](sparc-phase-field.md), the [framework page](../../../README.md) and the distance-fit pages, and the two senses meet whenever a cosmology page states both a background and an acceleration; the $`0`$ form is entrenched where the flat placement was pinned. Harmonizing toward $`a_\ast`$ is the expected direction, and is a separate decision from any single page's edit. Recorded here so the next page to state the normalization picks deliberately rather than by whichever neighbour it copied.
