...
**The measurement path for P2 (registered, pending).** Give the kernel an asymmetry $`\epsilon`$: flux $`\propto w_1^{1/2-\epsilon} w_2^{1/2+\epsilon}`$, still degree one, grade $`3/2 + \epsilon`$; then $`d\mu_{\text{tick}} \propto S^{3/2+\epsilon}\,dt`$ and $`dt/d\tau = S^{-1/2-\epsilon}`$. The fit reads this observable dial; it does not observe microscopic kernel symmetry directly. Every clock model already run is an $`\epsilon`$ slice:
