...
**Why this matters:** ΛR² = 3 is derived from the postulate (eigenvalue + Gauss equation; provenance in [the R problem](r-problem.md)). The coefficient 3 is the content. But Λ = 3/R² produces a number only with an independent R. Every previous route to R is either circular (de Sitter: R = √(3/Λ), feeds Λ back) or not an independent route to R (the Molien gap fixes the low-ℓ deficit boundary, a surviving spectral fact, but does not by itself pin R). The mass spectrum does determine R independently of Λ, the CMB, and the de Sitter relation, so the circularity is genuinely broken: Λ = 3/R² becomes a forward prediction from particle physics. The prediction lands within about half an order of magnitude of observed for the best assigned pair (muon-top), and within about one order for the electron-muon lepton pair, a real result at that bar but not the percent-level number Sections VI and VIII originally claimed.
