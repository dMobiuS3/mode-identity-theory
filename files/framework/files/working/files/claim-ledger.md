...
| 5b | α-formula rule conjunction: well / grid / exponent | 24-candidate restricted class; 7,200-combination broad control ([alpha-scan.test.py](alpha-scan.test.py)) | **chosen, audited (provisional)**: the broad control returns 9 hits against a ~7.6 local-density baseline, **consistent with that baseline**; the restricted class returns 1 hit, for which the source publishes no expected count, so it stands as a bare 1-of-24 rather than a measured significance. Positive content therefore lives in the restriction | α |
