...
| Gauss factor 3 (Ricci trace) × ½ → 3/2 | 3 derived, ½ imported GR | identity with stabilizer-3/2 is OPEN. The ½ is the VACUUM de Sitter normalization on a time-symmetric slice, which is the throat of an expanding solution, not a static state; vacuum GR admits no static closed solution. For a static closed domain with a homogeneous isotropic perfect fluid the unchanged Einstein equations give `ΛR² = 3 − 2/(1+w)`: 1 for dust, 3/2 for radiation, and 3 only as `w → ∞`, never attained. So 3 is a reference value, gated on the stress tensor of the mode content |
...
*Rows 5–8 also appear in Null results / Loose comparisons above; this is the same fact read next to its neighbors, not a second witness. Sourced at: [fine-structure.md](../../../../spectrum/files/fine-structure.md) §IV (row 5), the a₀(z) paper §2 / Fig 2.1 (row 6), [fibonacci-wells.md](fibonacci-wells.md) probe log (row 7), [mass-null-test.md](mass-null-test.md) tag `mass-null-v1.1` (row 8); rows 1–4 in [scaling-law-uniqueness.md](scaling-law-uniqueness.md).*
