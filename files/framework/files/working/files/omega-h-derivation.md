...
**Notation note added 2026-09-03, outside the frozen question; nothing below is edited.** The framework page now reserves $`\Omega_H`$ for the cosmological rate ledger $`(c/H\ell_P)^2`$ and writes this page's object as $`\Omega_\Psi`$, the local phase-gradient hierarchy built from $`\kappa_\Psi = \lvert u^\mu\nabla_\mu\ln\lvert\Psi\rvert\rvert`$. Every $`\Omega_H`$ below means $`\Omega_\Psi`$. The split was forced by arithmetic rather than taste: for any lapse $`d\tau = N\,dt`$ the factor $`N`$ cancels from the ratio of the two logarithmic gradients, so no congruence makes them one object, and their ratio is $`\Omega_\Psi/\Omega_H = (\Psi/S)^4`$, equal to $`1`$ only at budget equipartition $`\Psi^2 = S^2 = 1/2`$ and about $`3\times10^4`$ at the present epoch. That sharpens this page's own question rather than changing it: a congruence reconciling the two ledgers would have to produce exactly $`d\tau/d\tau_H = (S/\Psi)^2`$, so if the static and freely-falling channels differ by some other power of $`S/\Psi`$, the congruence choice is not what closes the gap. The frozen question, its premise and its $`p`$ exponents stand as written.
