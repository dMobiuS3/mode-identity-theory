...
Conjugacy classes of homomorphisms $`\pi_1(S^3/2I)=2I\to\mathrm{SU}(2)`$ correspond to flat $`\mathrm{SU}(2)`$ connections on $`S^3/2I`$ up to gauge. Up to conjugacy there are, besides the trivial one, exactly two irreducible classes, given by $`Q`$ and $`Q'`$; both are isolated and nondegenerate [FintushelStern]. We name flat connections by their representations. The complexified adjoint of the flat connection $`Q`$ is $`\mathrm{Sym}^2Q`$, and likewise for $`Q'`$: the Galois pair of two-dimensional representations induces the Galois pair of three-dimensional adjoints, at McKay distances $`2`$ and $`6`$. Among the binary polyhedral groups, $`2I`$ alone has an adjoint Galois pair that is distinct; perfectness is what rules out the octahedral coincidence, as Section 8 records.
