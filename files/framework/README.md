...
Cooling applies the same ratio to the bath: every wavelength rescales by $`S`$, photon number density by $`S^{-3}`$ at conserved photon count, radiation energy density by $`S^{-4}`$, and the Planck blackbody is preserved at $`T \propto 1/S`$. The fixed quantity is the spectral radius, not the effective volume; where the redshifted photon energy goes is left to the same stress-energy accounting still under construction. Read as an expansion history in the ΛCDM way, this dictionary carries the familiar shift from the matter-only $`q_0 = +0.5`$ to the observed $`q_0 \approx -0.55`$, and the [budget map](files/working/files/budget-map.md) keeps the full accounting.
