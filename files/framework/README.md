...
**Calibration closure: do the independent scale-fixings agree?** No. The coupling route puts the master length at $`R_\Lambda \approx 6.1`$ Gpc and the mass route at $`\approx 20`$ Gpc, a factor of 3.2 in $`R_\Lambda`$ and 10.5 in $`\Lambda_\text{ref}`$; read instead against the measured $`\approx 5.4`$ Gpc, the mass route is the fourteenfold miss the Inputs table records. Reconciling them is the open problem Inputs and Calibration states in full. Until one route is established as the correct $`R_\Lambda`$, every absolute scale that flows through it inherits the ambiguity.
