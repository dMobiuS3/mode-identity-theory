...
**Dynamics: how does the wave content source geometry?** Not yet by any derived map, and that gap is one object rather than several. It accounts for two symptoms that read as separate problems, gravity's open construction in [One Interface](#one-interface) and the clock exponent in [One Wave](#one-wave), and until it is built the two placements the coefficient constraint leaves open stay undecided. [One Interface](#one-interface) poses the problem; the stress-tensor bridge is the program that would close it. This is the framework's largest structural debt: several pages are waiting on one bridge.
