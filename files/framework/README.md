...
**The named anchor.** For the $`\Lambda`$-anchored default this repository uses Planck 2018 base-ΛCDM, TT,TE,EE+lowE+lensing: $`H_0 = 67.36`$ km s⁻¹ Mpc⁻¹ and $`\Omega_\Lambda^\text{(density)} = 0.6847`$, giving $`\Lambda\,\ell_P^2 = 2.845 \times 10^{-122}`$, hence $`\Omega_\Lambda = 1.054 \times 10^{122}`$ and $`R_\Lambda = 5.38`$ Gpc. Working prose rounds that row to $`H_0 = 67.4`$ and $`0.685`$, which returns the same $`2.85 \times 10^{-122}`$ to three figures. Everything absolute in the corpus derives from this one row: other Planck combinations, +BAO in particular, are comparison datasets and not silent replacements for the anchor.
...
**Calibration closure: do the independent scale-fixings agree?** No. The coupling route puts the master length at $`R_\Lambda \approx 6.1`$ Gpc and the mass route at $`\approx 20`$ Gpc, a factor of 3.2 in $`R_\Lambda`$ and 10.5 in $`\Lambda_\text{ref}`$; read instead against the measured $`\approx 5.4`$ Gpc, the mass route is the ~13.4× miss the Inputs table records. Reconciling them is the open problem Inputs and Calibration states in full. Until one route is established as the correct $`R_\Lambda`$, every absolute scale that flows through it inherits the ambiguity.
