...
The Möbius band $M$ is constructed as the quotient of a spherical band on the great $S^2 \subset S^3$ by an orientation-reversing isometry. The great $S^2$ is a totally geodesic submanifold of $S^3$; the Möbius band itself is not a totally geodesic submanifold of $S^3$ (totally geodesic submanifolds of $S^3$ are open subsets of great spheres and therefore orientable). The Möbius band inherits a locally totally geodesic metric from the covering spherical band: the intrinsic geometry at every smooth point is identical to that of $S^2$, carrying constant Gaussian curvature $K = 1/R^2$. The spectral problem is well-posed on this abstract Riemannian manifold. Throughout this section, $K_{ij}$ denotes the extrinsic curvature (second fundamental form) of $S^2$ in $S^3$, while $K$ denotes the intrinsic Gaussian curvature of the surface.

**The covering space.** Realize $S^3$ as the simply connected closed 3-manifold of constant sectional curvature $1/R^2$. The great $S^2 \subset S^3$ is a totally geodesic 2-sphere: every geodesic of $S^2$ is a great circle of $S^3$, so the second fundamental form vanishes ($K_{ij} = 0$). The spherical band $\widetilde{M}$ defined in Section 2 is an open subset of this $S^2$. The orientation-reversing isometry

$$(x_1, x_2, x_3, x_4) \mapsto (-x_1, -x_2, -x_3, x_4)$$

is an isometry of $S^3$ that restricts to the antipodal map on the equatorial $S^2$, sending each point of the spherical band to its antipode. The Möbius band $M$ is the quotient of $\widetilde{M}$ by this identification, equivalently by imposing the twisted identification $(0, w) \sim (\pi R, -w)$ from Section 2. The quotient is an abstract Riemannian manifold that is locally isometric to $S^2$ but globally non-orientable. Away from the cone point $y = \pi R/2$, the Möbius band is locally modeled on $S^2$ and carries the same locally totally geodesic metric. Its boundary consists of the two curves $w = \pm W$, which together form a single closed loop $\partial M = S^1$ traversing the band twice.

**The induced metric.** Because the covering $S^2 \subset S^3$ is totally geodesic, its intrinsic metric equals the restriction of the ambient metric and intrinsic geodesics coincide with ambient geodesics. The Möbius band inherits this metric through the quotient. The following calculation is intrinsic to $S^2(R)$. In Fermi coordinates on the totally geodesic $S^2 \subset S^3$, let $w$ denote arclength along a reference great circle and let $y$ denote arc-length parameter along a meridional geodesic of $S^2$, with $y = 0$ at the reference great circle. The normal Jacobi field satisfies
...
**The cone singularity.** At $y = \pi R/2$, the metric coefficient $g_{ww} = \cos^2(y/R)$ vanishes, so all points at that longitude are metrically identified. On the covering band $\widetilde{M}$, this is the smooth north pole of $S^2$ and the coordinate singularity is removable. On the quotient Möbius band $M$, the transverse interval collapses to a single point with cone angle $2W/R < 2\pi$ (for $W < \pi R$), producing a genuine metric singularity. In Sector $\mathcal{A}$, the reduction to an ODE in $y$ yields a Sturm-Liouville problem with a singular endpoint at $y = \pi R/2$, addressed by the Friedrichs extension (Section 2) and the excision argument (Section 6). The Gaussian curvature $K = 1/R^2$ extends continuously to the cone point on the smooth locus.

To see the cone structure explicitly, set $\delta = y - \pi R/2$ and $\rho = |\delta|$. Near $\rho = 0$ the metric takes the form $ds^2 \approx d\rho^2 + (\rho/R)^2\,dw^2$. With $w \in [-W, W]$ and Neumann conditions at the endpoints, the total angular range subtended is $2W/R$. For $W < \pi R$ this is strictly less than $2\pi$, confirming a metric cone with deficit angle $2\pi - 2W/R$. On the covering $S^2$, $w$ ranges over the full $2\pi R$ and the deficit vanishes; the quotient identification restricts the range and creates the singularity. The metric expression $ds^2 = dy^2 + \cos^2(y/R)\,dw^2$ is the same on both spaces because Fermi coordinates are agnostic about the range of $w$.
...
This is the curvature the flat strip does not carry. The $S^2$ quotient construction supplies it through the inherited constant curvature. The Laplace-Beltrami operator on this metric is the object of Sections 5 and 6.
