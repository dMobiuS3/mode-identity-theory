...
The closed forms interlock: the Galois pairs satisfy $`T^2(R_3)/T^2(R_4) = \varphi^{-4}`$ and $`T^2(R_1)/T^2(R_2) = \varphi^{-8}`$ exactly, and each pair swaps under $`\varphi \to -1/\varphi`$ as Galois conjugates must; the Galois-fixed irreps $`R_6`$ and $`R_7`$, $`R_5`$, $`R_8`$ carry the rational values $`1`$ and $`9/4`$, $`25/9`$, $`4`$. The integer-spin product $`T^2(R_3)\,T^2(R_7)\,T^2(R_5)\,T^2(R_4) = 4`$ and the half-integer product $`T^2(R_1)\,T^2(R_2)\,T^2(R_6)\,T^2(R_8) = 1/4`$ are exact inverses. All values are verified by the reproducible correction artifact ([torsion-correction](../../framework/files/working/files/torsion-correction.md), with its mutation-tested `.test.py`), and the full table has been reproduced by a context-isolated independent-method run on the OpenWave M8 track, from a based chain complex of $`S^3/2I`$ rather than the spectral-zeta definition used here. That run's native orientation is the reciprocal of the one on this page, a choice of direction in the definition of the torsion; under that single global inversion, selected at $`R_7`$ before any other row was compared, all eight nontrivial values agree exactly in $`\mathbb{Q}(\varphi)`$ and the two Galois ratios and two sector products follow. The protocol pre-registered exactly that inversion as the one admissible convention bridge, so the outcome carries the same claim as an identity-orientation match, with the orientation recorded. The supplied topological model was verified by the run, not independently derived ([adjudication record](https://github.com/openwave-labs/openwave/blob/main/openwave/xperiments/m8_mit/research/findings/m8_8_adjudication_record.md)).
...
A pre-registered null test quantified the table's counting caveat. The first run (`mass-null-v1.0`, July 11, 2026) was registered against the pre-correction table, whose archived inputs carry the coexact-only torsion values; after the correction moved twelve entries, the identical frozen procedure was re-registered and re-run against the corrected table (`mass-null-v1.1`, July 28, 2026). Randomly reassigning the corrected torsion factors across the fixed quantum-number slots reproduced or exceeded the observed compatible-coverage score in 69.0% of draws ($`p_A = 0.690`$), the null distribution now centered on the observed count (mean 5.02 against the observed 5). The ×3 proximity count is therefore uninformative about whether the specific torsion values occupy the correct slots: the corrected count is the typical outcome of a random torsion assignment.
