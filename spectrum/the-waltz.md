...
The cosmological constant Λ lives on the Mobius surface as a topological eigenvalue. Observation infers it from three-dimensional dynamics in $S^3$. The Gauss-Codazzi equations provide the interface: a $3/2$ conversion factor that carries 2D surface curvature into 3D spatial geometry. The same interface governs masslessness and reframes black holes as nodes where sampling amplitude vanishes.
...
At the zero-crossings (nodes) of the standing wave, $C(\Theta) \to 0$ and sampling ceases. The wave persists. The topology holds. The surface is real, but the channel from the edge to the surface has closed, and GR reports infinity because the conversion divides by zero amplitude.
...
Information is not destroyed; it is unsampled. The wave continues, the record continues, and no observer at finite $\sqrt{\Omega}$ can read it.
