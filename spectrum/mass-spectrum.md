...
The vertex stabilizer encodes the electroweak interface between the Galois pair of vacua. How that interface assigns $T_3$ and $Y$ to specific modes is current work, with the character values at order-10 elements (half-angle $\pi/5$, the dodecahedral vertex angle) and the $\cos(\pi/10)$ projection as the leading structural ingredients.
